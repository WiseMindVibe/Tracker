...
export default function TrafficCredentialsRepeater({
    catalogId,
    definitions,
    value,
    onChange,
    ...
    if (!catalogId) {
        return (
            <p className="text-sm text-muted-foreground mt-4">
                Select a Traffic Source to configure credentials.
            </p>
        );
    }
    ...
    if (defs.length === 0) {
        return (
            <p className="text-sm text-muted-foreground mt-4">
                No credential fields defined for this traffic source.
            </p>
        );
    }
        ...
        <div className="mt-6 space-y-3">
            ...
            {value.map((row, index) => (
                <div key={row.key}>
                    ...
                    />
                    {errors[`trafficCredentials.${index}.value`] && (
